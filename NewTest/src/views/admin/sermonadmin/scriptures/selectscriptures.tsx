...
const { data: scriptureData, error } = Fetch<ScriptureType[]>(
  ...
  const selectedScripture = scriptureData?.find((scripture) => scripture.slug === selectedId);
  const sortedScriptureData = scriptureData?.sort((a, b) => {
    return bibleOrder.indexOf(a.book) - bibleOrder.indexOf(b.book);
  });
  if (!scriptureData || error ||  scriptureData===undefined || scriptureData.length===0) {
        ...
        <PopoverTrigger asChild>
          ...
          >
            {selectedId && selectedScripture
               ? sortedScriptureData?.find((scripture) => scripture.slug === selectedId)?.book
               : `Select scripture...`}
              ...
              {scriptureData.map((scripture) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    setOpen(false)
                      ...
                      dispatch(setSelectedScripture(currentValue !== selectedId ? scripture : null));
                ...
                >
                  ...
                  <CheckIcon
                    className={cn(
                      "mr-2 h-4 w-4",
                      selectedId === scripture.slug ? "opacity-100" : "opacity-0"
                    )}
                  />
                </CommandItem>
              ))}
